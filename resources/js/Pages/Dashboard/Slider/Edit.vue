...
export default {
    ...
    data() {
        return {
            ...
            sliderPages: [
                ...
                {
                    label: "Подробно 'Полезный контент'",
                    value: "post.single",
                },
                {
                ...
                {
                    label: "Подробно 'Проекты'",
                    value: "project.single",
                },
                {
                    label: "Ипотека",
                    value: "ipoteka",
                ...
                {
                    label: "Строительство из бруса",
                    value: "brus",
                },
                {
                    label: "Модульное строительство",
                    value: "module",
                },
                {
                    label: "Ручная рубка",
    ...
    methods: {
        changeImage($event, item) {
            if ($event.target.files.length > 0) {
                ...
                if (input.files && input.files[0]) {
                    ...
                    reader.onload = (e) => {
                        this.item[item] = e.target.result;
                    };
        ...
        validLink() {
            if (this.item.link === null || this.item.link.length == 0)
                return true;
            else if (
                this.item.link != null &&
                this.item.link.startsWith("http")
            )
                return true;
            return false;
        },
        ...
        submit() {
            axios
                .put(route("sliders.update", this.slider.id), this.item)
                .then((res) => {
                    Swal.fire({
                        title: `Слайд успешно обновлен`,
                        timer: 3000,
                    ...
                    router.visit(route("sliders.index"), { replace: true });
                })
                .catch((err) => {
...
<template>
    <DashboardLayout>
        <div class="mb-6">
            <h2 class="font-bold text-xl">Редактировать слайд</h2>
        </div>
        <form class="sm:grid sm:grid-cols-2 gap-6" @submit.prevent="submit()">
            ...
            <div class="col-span-2 border-t-2 pt-3">
                <label class="block" for="">
                    ...
                    <span class="text-xl text-red-500"
                        >*
                        <span class="text-sm"
                            >(рекомендуемый размер 1500x800 пиксель)</span
                        ></span
                    >
                </label>
                <input
            ...
            <div class="col-span-2 border-t-2 pt-3">
                <label class="block" for="">
                    Изображение для мобильного версия сайта:
                    <span class="text-xl text-red-500"
                        >*
                        <span class="text-sm"
                            >(рекомендуемый размер 450x730 пиксель)</span
                        ></span
                    ></label
                >
                <input
                    id="mobile_image"
                    @change="changeImage($event, 'mobile_image')"
                    type="file"
                    accept="image/*"
                    :required="item.mobile_image == ''"
                    class="w-[1px] h-[1px] opacity-0"
                />
                <label for="mobile_image" class="cursor-pointer">
                    <div v-if="item.mobile_image" class="relative group">
                        <img
                            class="object-cover h-[270px] rounded-lg border border-dashed border-gray-900/25 w-full"
            ...
            <div class="">
                <label class="block" for="">Ссылка:</label>
                <input
                    type="text"
                    class="form-controls"
                    ...
                    v-model="item.link"
                />
                <span v-if="!validLink()" class="text-red-500 mt-2"

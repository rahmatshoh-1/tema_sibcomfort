...
export default {
    ...
    mounted() {
        this.item = this.review;
        console.log((this.item = this.review));
    },
    ...
    methods: {
        ...
        submit() {
            axios
                .put(route("reviews.update", this.review.id), this.item)
                .then((res) => {
                    Swal.fire({
                    ...
                    router.visit(route("reviews.index"), { replace: true });
                })
                .catch((err) => {
...
<template>
    <DashboardLayout>
        <div class="mb-6">
            <h2 class="font-bold text-xl">Редактировать отзыв</h2>
        </div>
        <form class="sm:grid sm:grid-cols-2 gap-6" @submit.prevent="submit()">
            <div class="">
                <label class="block" for="">Имя :</label>
                <input
                    type="text"
                    class="form-controls"
                    required
            ...
            <div class="col-span-2 border-t-2 pt-3">
                <label class="block" for="">Контент:</label>
                <ckeditor
                    v-model="item.body"
                    :editor="editor"
                    :config="editorConfig"
            ...
            <div class="col-span-2 border-t-2 pt-3">
                <label class="block" for=""> Изображение: Автора </label>
                <input
                    id="image"
            ...
            <div class="col-span-2 border-t-2 pt-3">
                ...
                <div class="flex items-center justify-end gap-x-3">
                    <Link
                        ...
                        >Назад</Link
                    >
                    <button class="dash-btn" :href="route('reviews.index')">
                        Сохранить
                    </button>
                </div>
            </div>

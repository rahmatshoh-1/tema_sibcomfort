<template>
    ...
    >
        <Link
            ...
            class="relative mx-3 mt-3 flex h-52 overflow-hidden"
        >
            <img
                v-tilt="{ speed: 500, perspective: 1200 }"
                class="object-cover w-full transition-all rounded group-hover:scale-110"
                :src="product.image"
                alt="product image"
            />
            <div
                class="bg-white rounded-r-[4px] absolute left-0 bottom-6 px-2 py-1 text-[#16A34A] text-[12px]"
            >
                {{ product.title }}
            </div>
        </Link>
        <div class="!mt-4 !px-5 !pb-5">
            <Link :href="route('product.single', product.id)">
                <h5
            ...
            <div class="mb-5 text-base">
                <p class="mb-1 line-clamp-3" v-html="product.body"></p>
            </div>
        ...
        <template v-if="showModal">
            ...
            >
                <div
                    class="p-8 mb-4 bg-[#434343] text-white rounded relative z-[1001] mx-4"
                >
                    <button
                        type="button"
                    ...
                    <form @submit.prevent="submit" class="group">
                        ...
                        <p
                            class="text-center max-w-xs mx-auto mt-4 text-gray-300"
                        >
...
export default {
    ...
    methods: {
        submit() {
            this.item.type = this.productType;
            this.item.nameProduct = this.product.name;
            this.item.specialOffer = this.product.title;
